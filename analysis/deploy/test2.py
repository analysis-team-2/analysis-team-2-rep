...
# AutoGluon 로깅 비활성화
    ...
    try:
        ...
        # JSON 데이터를 개별 열로 처리하여 DataFrame 생성
        df_predict = pd.DataFrame({
            'admi_cty_no': data['admi_cty_no'],
            'card_tpbuz_cd': data['card_tpbuz_cd'],
            'amt': data['amt'],
            'cnt': data['cnt'],
            'TOTAL_POPULATION': data['TOTAL_POPULATION'],
            '운영점포평균영업기간': data['store_avg_period'],
            '폐업점포평균영업기간': data['closure_avg_period'],
            '상권변동지표구분': data['business_change_indicator']
        })
